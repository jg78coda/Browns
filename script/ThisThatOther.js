...
request.open('GET', 'json/ThisThatOther.json', true);
...
function addQandA(data) {
		...
		for ( i = 0; i < datafeed.questions.length; i++) {
			...
			//var qtextnode = document.createTextNode(thequestion); this made Tags as text
...
var game = {
	...
	checkScoring: function(yards){
		...
		//sessionStorage.setItem("2", this.score);
	},
	...
	openModal: function(yards) {
		...
		if ( yards == 10 ) {
			...
			TweenMax.from(modalContent, .75, {opacity:0, y:150, ease: Elastic.easeOut.config(.75,.3)});
	...
	menuModal: function() {
		var menuModal = document.getElementById('menuModal');
			...
			TweenMax.from(menuModalContent, .75, {opacity:0, y:150, ease: Elastic.easeOut.config(.75,.3)});
	},
